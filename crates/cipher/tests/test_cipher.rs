...
#[test]
fn aes_gcm_different_keys_produce_undecryptable_ciphertexts() {
    // AEAD encrypt uses a fresh random nonce per call, so we cannot compare
    // ciphertexts directly. Instead, the stronger property: ciphertext
...
fn aes_gcm_tampered_nonce_fails_to_decrypt() {
    ...
    // Mutating the nonce must cause AES-GCM to fail authentication (and not
    // silently produce a wrong plaintext).
    ct.nonce[0] ^= 0xFF;
    assert!(
        cipher.decrypt(&ct).is_err(),
