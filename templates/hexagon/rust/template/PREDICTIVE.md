...
tier (`hexa-*-lib`, `phenotype-*-sdk`, `phenotype-*-framework`) or extracts
a capability from an app-level repo into a substrate crate.

The CI lint (`hexa-ci-templates/predictive-dry-check.yml`) requires the 4
...
each ADR-042 gate was met. See `hexa-ci-templates/PROMOTION.md` template.
...
- `hexa-predict` — fleet-wide similar-code scanner
- `hexa-drift-detector` — app-substrate drift scanner
- `hexa-framework-lint` — tier-convention enforcer